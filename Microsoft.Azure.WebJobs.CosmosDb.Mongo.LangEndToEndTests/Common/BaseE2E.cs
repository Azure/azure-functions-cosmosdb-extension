...
public class BaseE2E
{
	...
	protected BaseE2E(Language language,
	...
	{
		var storageQueueName = Utils.BuildStorageQueueName(appType, _language);
		...
		KafkaEntity queueEntity)
	{
		if (httpRequestEntity != null && InvokeType.HTTP == invokeType)
		{
			...
			{
				_logger.LogError(
					$"Unable to invoke functions for language:{_language} with exception {ex}");
				throw ex;
			}
		}
		else
